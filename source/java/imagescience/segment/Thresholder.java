...
public class Thresholder {
	...
	*/
	...
	/** Applies hysteresis thresholding to an image.
		...
		@param low The lower threshold value of the hysteresis.
		
		@param high The higher threshold value of the hysteresis.
		
		<p>Elements with a value larger than or equal to {@code high} are set to {@code 255}, elements with a value less than {@code low} are set to {@code 0}, and elements with a value larger than or equal to {@code low} are set to {@code 255} only if they are connected to elements with a value larger than or equal to {@code high} through elements all with a value larger than or equal to {@code low}. If the size of the image in the z-dimension equals {@code 1}, this method uses 8-connectivity in x-y space, otherwise it uses 26-connectivity in x-y-z space. The algorithm is applied to every x-y(-z) subimage in a 5D image.</p>
