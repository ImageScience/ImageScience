...
public class FloatImage extends Image {
	...
	*/
	...
	/** Wrapper constructor.
		...
		@throws IllegalArgumentException If the image elements of {@code imageplus} are not of type {@code float}.
	...
	public void set(final Coordinates coords, final float[] values) {
		...
		switch (axes) {
			...
			case Axes.T: {
				...
				final int exy = coords.y*dims.x + coords.x;
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					elements[coords.c][et][coords.z][exy] = values[vt];
				break;
			}
			case Axes.C: {
				int vcstart = 0;
				...
				final int exy = coords.y*dims.x + coords.x;
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					elements[ec][coords.t][coords.z][exy] = values[vc];
				break;
			}
			default:
				throw new IllegalStateException("Wrong number of active axes");
	...
	public void set(final Coordinates coords, final float[][] values) {
		...
		switch (axes) {
			...
			case Axes.TX: {
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
						elements[coords.c][et][coords.z][ex] = values[vt][vx];
				break;
			}
			case Axes.ZY: {
				int vystart = 0;
			...
			case Axes.TY: {
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
						elements[coords.c][et][coords.z][ey] = values[vt][vy];
				break;
			}
			case Axes.TZ: {
				int vzstart = 0;
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						elements[coords.c][et][ez][exy] = values[vt][vz];
				break;
			}
			case Axes.CX: {
				int vxstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
						elements[ec][coords.t][coords.z][ex] = values[vc][vx];
				break;
			}
			case Axes.CY: {
				int vystart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
						elements[ec][coords.t][coords.z][ey] = values[vc][vy];
				break;
			}
			case Axes.CZ: {
				int vzstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						elements[ec][coords.t][ez][exy] = values[vc][vz];
				break;
			}
			case Axes.CT: {
				int vtstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						elements[ec][et][coords.z][exy] = values[vc][vt];
				break;
			}
			default:
				throw new IllegalStateException("Wrong number of active axes");
		...
		switch (axes) {
			...
			case Axes.TYX: {
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
						for (int x=exstart, ex=ey, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
							elements[coords.c][et][coords.z][ex] = values[vt][vy][vx];
				break;
			}
			case Axes.CYX: {
				int vxstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
						for (int x=exstart, ex=ey, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
							elements[ec][coords.t][coords.z][ex] = values[vc][vy][vx];
				break;
			}
			case Axes.TZX: {
				int vxstart = 0;
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
							elements[coords.c][et][ez][ex] = values[vt][vz][vx];
				break;
			}
			case Axes.CZX: {
				int vxstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
							elements[ec][coords.t][ez][ex] = values[vc][vz][vx];
				break;
			}
			case Axes.CTX: {
				int vxstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
							elements[ec][et][coords.z][ex] = values[vc][vt][vx];
				break;
			}
			case Axes.TZY: {
				int vystart = 0;
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
							elements[coords.c][et][ez][ey] = values[vt][vz][vy];
				break;
			}
			case Axes.CZY: {
				int vystart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
							elements[ec][coords.t][ez][ey] = values[vc][vz][vy];
				break;
			}
			case Axes.CTY: {
				int vystart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
							elements[ec][et][coords.z][ey] = values[vc][vt][vy];
				break;
			}
			case Axes.CTZ: {
				int vzstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
							elements[ec][et][ez][exy] = values[vc][vt][vz];
				break;
			}
			default:
				throw new IllegalStateException("Wrong number of active axes");
			...
			case Axes.TZYX: {
				...
				for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
							for (int x=exstart, ex=ey, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
								elements[coords.c][et][ez][ex] = values[vt][vz][vy][vx];
				break;
			}
			case Axes.CZYX: {
				int vxstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
							for (int x=exstart, ex=ey, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
								elements[ec][coords.t][ez][ex] = values[vc][vz][vy][vx];
				break;
			}
			case Axes.CTYX: {
				int vxstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
							for (int x=exstart, ex=ey, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
								elements[ec][et][coords.z][ex] = values[vc][vt][vy][vx];
				break;
			}
			case Axes.CTZX: {
				int vxstart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
							for (int x=exstart, ex=exy, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
								elements[ec][et][ez][ex] = values[vc][vt][vz][vx];
				break;
			}
			case Axes.CTZY: {
				int vystart = 0;
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
							for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
								elements[ec][et][ez][ey] = values[vc][vt][vz][vy];
				break;
			}
			default:
				throw new IllegalStateException("Wrong number of active axes");
				...
				for (int ec=ecstart, vc=vcstart; ec<ecstop; ++ec, ++vc)
					for (int et=etstart, vt=vtstart; et<etstop; ++et, ++vt)
						for (int ez=ezstart, vz=vzstart; ez<ezstop; ++ez, ++vz)
							for (int y=eystart, ey=exy, vy=vystart; y<eystop; ++y, ey+=dims.x, ++vy)
								for (int x=exstart, ex=ey, vx=vxstart; x<exstop; ++x, ++ex, ++vx)
									elements[ec][et][ez][ex] = values[vc][vt][vz][vy][vx];
				break;
			}
			default:
				throw new IllegalStateException("Wrong number of active axes");
